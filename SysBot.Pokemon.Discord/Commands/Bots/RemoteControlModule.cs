...
namespace SysBot.Pokemon.Discord
{
    ...
    public class RemoteControlModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task ClickAsync(SwitchButton b)
        {
            ...
            if (bot == null)
            {
                await ReplyAsync($"No hay bot disponible para ejecutar su comando: {b}").ConfigureAwait(false);
                return;
            }
        ...
        [Command("setStick")]
        [Summary("Establece la palanca en la posición especificada.")]
        [RequireRoleAccess(nameof(DiscordManager.RolesRemoteControl))]
        public async Task SetStickAsync(SwitchStick s, short x, short y, ushort ms = 1_000)
        {
            ...
            if (bot == null)
            {
                await ReplyAsync($"No hay bot disponible para ejecutar su comando: {s}").ConfigureAwait(false);
                return;
            }
        ...
        private async Task SetScreen(bool on, string ip)
        {
            ...
            await ReplyAsync("Estado de Pantalla: " + (on ? "On" : "Off")).ConfigureAwait(false);
        }
